 ...
 * column) a region holds, rolled up so a parent's count is the log₂-sum of its children (capped at
 * {@link RegionAddress#MAX_COARSE_LEVEL}). Each column below reads the count for the <b>region containing the
 * bot</b> at one level — and because those regions nest (a level-2 cell contains the bot's level-1 cell,
 * etc.), the counts grow left-to-right:
 * <ul>
 ...
 *   <li>{@code l4+} — the coarsest {@code 1024}-block tally the pyramid rolls up to: the "global" count.</li>
 * </ul>
...
public final class ReportCommand implements BotCommand {
    ...
    /** Pyramid levels shown, coarsest last (the "global" column). Sizes: 16&lt;&lt;level blocks/side. */
    private static final int[] LEVELS = { 1, 2, 3, RegionAddress.MAX_COARSE_LEVEL };
    /** Column headers aligned to {@link #LEVELS}. */
    private static final String[] HEADS = { "l1", "l2", "l3", "l4+" };
    ...
    /** Index of the "global" column within {@link #LEVELS} (the one the resource filter/sort keys on). */
    private static final int GLOBAL_COL = LEVELS.length - 1;
    ...
    public void contribute(LiteralArgumentBuilder<CommandSourceStack> bot) {
        bot.then(Commands.literal("report").executes(ctx -> OrebitCommands.act(ctx, (b, player, src) -> {
            ...
            // Gather every resource the bot knows about (nonzero coarsest tally), with its per-level bytes.
            final List<int[]> rows = new ArrayList<>(); // {col, e[l1], e[l2], e[l3], e[global]}
            for (int col = 0; col < ResourceClasses.COLUMN_COUNT; col++) {
                final int[] e = new int[LEVELS.length + 1];
                e[0] = col;
                for (int i = 0; i < LEVELS.length; i++) {
                    e[i + 1] = countByte(pyramid, minY, at, LEVELS[i], col) & 0xFF;
                }
                if (e[GLOBAL_COL + 1] > 0) rows.add(e); // only list resources the compass has actually seen
            }
            ...
            // Most-abundant first (by the coarsest tally; larger log₂ byte == larger count).
            rows.sort((a, c) -> Integer.compare(c[GLOBAL_COL + 1], a[GLOBAL_COL + 1]));

            CommandFeedback.send(src, "Resource knowledge around you — l1/l2/l3 = the " + NEAR_SIDE
                    + "/64/128-block region you're in, l4+ = global (1024-block) tally. Counts approximate.");
