...
public class TraversalGrid {
    ...
    // (The old section-level `anyDoor` prefilter lived here — measured perf-useless by on/off isolation and
    // REMOVED per DESIGN-trapdoors.md §8: the per-pop openable context is now an unconditional feet-cell
    // descriptor read, and no anyTrapdoor/anyOpenable section bit may replace it — hard rule.)
