...
public final class ResourcePyramid {
     ...
     * ({@link com.orebit.mod.worldmodel.persistence.ResourcePyramidCodec#bucketShard}). Tick-thread only (interning is).
     */
    private final com.orebit.mod.worldmodel.hpa.ShardRowIndex shardIndex =
            new com.orebit.mod.worldmodel.hpa.ShardRowIndex();
    ...
    public int rowFor(int level, int rx, int ry, int rz) {
        ...
        if (row == before && level <= RegionAddress.SHARD_LEVEL) {
            // Newly created shard-level row → index it under its L5 shard (matches ResourcePyramidCodec.shardKeyOf /
    ...
    /**
     * Number of shard-indexed resource rows for L5 shard {@code shardKey} at shard-level {@code sl}
     * (0..{@link RegionAddress#SHARD_LEVEL}) — the persistence flush's per-shard gather seam
     * ({@link com.orebit.mod.worldmodel.persistence.ResourcePyramidCodec#bucketShard}). Includes interned-but-unbuilt /
    ...
    /** The {@code i}-th shard-indexed resource row (ascending intern order) for {@code shardKey} at shard-level {@code sl}. */
