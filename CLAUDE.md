...
- **Era branches** — `main` (newest era; GitHub default = "builds latest") and `mc-1.20` / `mc-1.16` / … (older). Each = `core` + **one** commit that adds the era-owned **`era.properties`** (the Loom version + the Stonecutter version-matrix; `java.version` recorded for reference). The Gradle wrapper version (`gradle/wrapper/gradle-wrapper.properties`) is era-owned by nature. Each branch is buildable; `runClient`/dev happens **here** (or in a git worktree).
...
**CURRENT STATUS: split executed (Model C step 1 done).** Two branches now exist: **`core`** (version-portable logic + overlays + `versions/*/gradle.properties` + build-script logic; **no toolchain values, not buildable** — a missing `era.properties` makes Gradle fail fast) and **`main`** (= `core` + the single `era.properties` commit for the ≤1.21.11 era: Loom 1.13.469 / Gradle 8.12.1 / JDK 21, MC 1.20.1→1.21.11). `main` is the newest/current era and the intended GitHub default. The toolchain values were externalized: `era.properties` (Loom version via `settings.gradle.kts` pluginManagement `resolutionStrategy`; the matrix via `versions()`/`branch()` reading `mc.versions.*`). **Backward proven (session 8):** the current toolchain reaches **1.14.4** (no wall down to it); the wall is **1.13.2** (no official Mojang mappings + the 1.13 Flattening → a separate pre-1.14 era, deferred). The current era was **extended back to 1.17.1 as real Fabric+Forge builds** (session 9): floor 1.17.1 (1.17.0 dropped — no fabric-api; 1.20.0 added), Fabric+Forge+common all compile 1.17.1→1.21.11, and a per-version **Java toolchain** (JDK 17 `<1.20.5`, JDK 21 `≥1.20.5`; both installed). **Runtime-verified in-game (session 9–10):** Fabric on 1.17.1/1.18.1/1.19/1.19.3/1.20/1.20.1 and **Forge on 1.17.1 + 1.18.1** (1.18.1 confirmed on a real *recommended*-Forge install → build-against-latest runs-on-recommended). Forge build pin = **latest** (Loom 1.13.469 corrupts some older recommended MDKs); the "Forge Beta" banner is cosmetic. Backward wall = **1.13.2**; ≤1.16.5 deferred (WIP on `scratch/backward-probe`). **NEXT = the 26.x forward era**: a NEW toolchain-era branch (`mc-26`) — needs Java 25 + a newer Loom (1.13.469 can't build 26.x) + Gradle 9; version files staged under `versions/26.*` (but their Java-25 toolchain + the `build.gradle.kts` `>=26 → 25` branch are TODO). See `HANDOFF.md` + BUILD-STRATEGY §3/§6/§8. **GOTCHA:** loader nodes build from `chiseledSrc` — run `Set active project to <ver>` before `runClient`/single-jar build after any src/overlay edit. The authoring rules above are live — honor them.
