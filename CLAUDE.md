...
- **worldmodel/navblock/** — REAL. `NavBlock` (compresses ~900 blocks into ≤256 behavioral fingerprints at static-init).
- **worldmodel/pathing/** — MIXED. REAL: `TraversalClass`, `TraversalGrid` (2-bit-packed 16³), `TraversalAnalyzer`/`TraversalAnalyzerMutable` (drifted; the Mutable one uses a `Set<Block>` + `ConcretePowder.all()`), `NavSection`/`NavSectionPool`, `ChunkNavBuilder`/`ChunkNavLoader` (call `ChunkCoords`). BROKEN: `NavSectionBuilder.build()` is an inert benchmark; `ChunkNavLoader` builds NavSection[] then **discards** it. STUBS: `ReflectionHelper`, `WorldModel`.
- **worldmodel/region/** — REAL data layer (`Region`/`LeafRegion`/`CompositeRegion`, `Portal`, `RegionPool`, `RegionMetadata`, `RegionBlockIndex`, `RegionBoundingBox`). STUB: `RegionBuilder` (flood-fill — empty).

STUB-ONLY (Javadoc + package decl, ZERO code): **pathfinding/** (28), **ai/** (9), **tasks/** (7), **sim/** (5), **memory/** (9), **relationships/** (5), **integration/** (10, the LLM pipeline), **behavior/**, **requirements/**, **settings/**, **config/**, **eventbus/** + **clock/**, **commands/** (5) + **debug/** (7), **manager/**, **agent/**, **data/**, **mocks/** (4) + **scripts/** (3). **Note duplicate identities**: `manager.BotManager` (stub) vs root `BotManager` (real); `mocks/` Mock* vs root Fake*; `PathfindingSettings` in BOTH pathfinding/ and settings/.
