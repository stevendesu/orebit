...
vertical keeps walk's dy shaping except fully-submerged ¬S·W ≈ 2.2×walk; ¬S·¬W pure air gets the relocated
...
- §3 connectivity = 6-connected flood fill over passable cells + fragment cap. *Amended: the occupiability
  ...
  id 62 unused/reserved, id 63 = the search-only `RegionPathfinder.VIRTUAL_GOAL_FRAG` (see HPA-CASCADE.md
  "The virtual goal fragment"). The persisted 6-bit COUNT field spends 63 on the
...
- §5 storage schema (packed fragment records inside the pyramid). *Wire form now
  `CostPyramidCodec` v7: per MIXED fragment, 2 type bits (S,W) between faceMask and footprints; uniform
