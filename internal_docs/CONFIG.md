...
| `pathing.boxedInScanRadius` | int `1..16` | `3` | Half-extent (in region cells, applied at every pyramid level `0..MAX_COARSE_LEVEL`) of the **proactive boxed-in seal scan** (`PathPlan.maybeProactiveBoxedIn`). → `BotCaps.boxedInScanRadius`. At each level the goal-rooted sealed-probe floods a `(2R+1)³` region box centred on the goal region: `R=3` (default) = a 7-region box catching seals from ~16 blocks (L0) to ~7k blocks (L6, 1024-block regions). Larger = catches bigger tombs but scans a wider box each time the bot picks a goal — a **plan-entry** cost (each per-level field is `(2R+1)³ · MAX_FRAGMENTS` floats and the flood is box-bounded), never a per-node one. Like `maxNodes`/`greedyWeight` it is a search knob riding on `BotCaps`, but is excluded from the realizability signature. | `pathing.boxedInScanRadius=5` |
...
### `doors.*` — how does the bot deal with doors (and trapdoors and fence gates) in its path?

| Key | Type / range | Default | Meaning | Example |
| --- | --- | --- | --- | --- |
| `doors.toggle` | boolean | `true` | The bot may OPEN/CLOSE hand-toggleable **doors, trapdoors AND fence gates** (wood/copper doors+trapdoors, every gate — the owner-ratified single family key, DESIGN-trapdoors.md §5, DESIGN-fence-gates.md §3) by right-clicking, preferring that over smashing them or routing around. **The only ability-style key that rides `BotCaps`** — `Config.toBotCaps()` folds it into `BotCaps.mayToggleDoors`, which `MovementContext.mayToggleDoors`/`doorSetClears`/`canToggleExitDoor` (and the trapdoor twins `trapdoorSetClears`/`trapdoorSetFloors`/`trapdoorSetClearsVertical`, and the face-agnostic gate arm `gateSetClears`) gate the planner's cheap toggle edit on, and which occupies **bit 3 of the realizability signature** (`BotCaps.realizabilitySig`) — so a toggles-on and a toggles-off search record distinct crossing theorems. Default `true` (DOORS P3: the follower operates doors for real — opens a closed door before crossing, closes it again on a hallway-corner exit; trapdoors ride the same executor pre-pass — open a hatch to drop through, close one into a floor; fence gates too — opened to walk through, never closed behind). Set `false` as a kill-switch to fall back to P1 behaviour (an already-open door/hatch/gate is walked through, a closed one is mined — or routed around when protected; note a closed FENCE GATE then becomes a **hard wall** under the default config, since `#minecraft:fence_gates` is in `mining.protectedBlocks` and the toggle was the only way through — DESIGN-fence-gates.md §3). Iron doors and iron trapdoors are never hand-toggleable regardless; no iron fence gate exists. Note the OPEN/CLOSE path never consults the PROTECTED bit, so protecting doors/trapdoors/gates (the default set does) blocks BREAKING them only, not operating them. | `doors.toggle=false` |
...
| `crafting.reclaimTable` | boolean | `true` | After crafting on a temporary table the bot itself placed, break it (timed, real drops) and take it back. A deliberate task break — `mining.protectedBlocks` is a PATHING policy and never gates the hands (owner ruling), so no waiver machinery is needed. `false` → placed tables stay in the world and the bot says where. | `crafting.reclaimTable=false` |
...
| Key | Type / range | Default | Meaning | Example |
| --- | --- | --- | --- | --- |
| `farming.workRadius` | int `4..48` | `16` | Horizontal half-extent (blocks) of the box a farm pass surveys around where the command was issued (vertical band fixed ±4). `/bot farm` is a PERSISTENT, follow-like state: each work cycle harvests every mature known crop in the box (a deliberate task break — maturity is the farmer's own rule), replants/plants bare farmland from carried seeds, sweeps up its own drops (item-lifecycle, no timers) — then WATCHES the farm, re-surveying on a 10s debounce so later-maturing crops get tended too, until another command switches the mode. Executor-read (`BotFarmer`), fully hot. | `farming.workRadius=8` |
| `farming.till` | boolean | `true` | A work cycle may TILL new ground (grass/dirt/dirt path, air above, water in the vanilla 9×9×2 hydration box — Chebyshev 4 at the cell's Y and Y+1, waterlogged counts) when the bot carries a hoe AND seeds — i.e. it may EXPAND the farm. `false` = tend existing farmland only. Tilling/planting run the REAL vanilla use path (`ItemStack#useOn` — durability, sounds, its own gates) and are verified by re-reading the block. | `farming.till=false` |
...
- `maxNodes = pathing.syncSearchBudgetNodes`, `greedyWeight = pathing.greedyWeight`,
  `boxedInScanRadius = pathing.boxedInScanRadius` (the proactive boxed-in seal-scan box half-extent
