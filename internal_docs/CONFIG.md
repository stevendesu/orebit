...
### `doors.*` — how does the bot deal with doors (and trapdoors) in its path?

| Key | Type / range | Default | Meaning | Example |
| --- | --- | --- | --- | --- |
| `doors.toggle` | boolean | `true` | The bot may OPEN/CLOSE hand-toggleable **doors AND trapdoors** (wood/copper — the owner-ratified single family key, DESIGN-trapdoors.md §5) by right-clicking, preferring that over smashing them or routing around. **The only ability-style key that rides `BotCaps`** — `Config.toBotCaps()` folds it into `BotCaps.mayToggleDoors`, which `MovementContext.mayToggleDoors`/`doorSetClears`/`canToggleExitDoor` (and the trapdoor twins `trapdoorSetClears`/`trapdoorSetFloors`/`trapdoorSetClearsVertical`) gate the planner's cheap toggle edit on, and which occupies **bit 3 of the realizability signature** (`BotCaps.realizabilitySig`) — so a toggles-on and a toggles-off search record distinct crossing theorems. Default `true` (DOORS P3: the follower operates doors for real — opens a closed door before crossing, closes it again on a hallway-corner exit; trapdoors ride the same executor pre-pass — open a hatch to drop through, close one into a floor). Set `false` as a kill-switch to fall back to P1 behaviour (an already-open door/hatch is walked through, a closed one is mined — or routed around when protected). Iron doors and iron trapdoors are never hand-toggleable regardless. Note the OPEN/CLOSE path never consults the PROTECTED bit, so protecting doors/trapdoors (the default set does) blocks BREAKING them only, not operating them. | `doors.toggle=false` |
