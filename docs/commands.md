...
|:--------|:-------------|
| `/bot spawn` | Respawn a dead or missing bot. Your bot is spawned for you automatically when you join, so this is the recovery hatch — it refuses while a live bot already exists. Inventory is lost exactly as it would be on a rejoin. |
...
| `/bot report` | Dump the bot's resource knowledge (the "compass") as a table: each resource it has mapped, with approximate counts at a few scales — `near`/`mid`/`far` (within ~64 / ~512 / ~8192 blocks of **the bot**, full depth) out to `global` (everything it has **ever explored, anywhere** — even resources it walked past thousands of blocks away). The near/mid/far windows are boxes centered on the bot rather than readings of whichever fixed grid cell it happens to stand in, so the numbers stay steady as it moves a few blocks (no jump at the world origin). The table covers the dimension the bot is currently in. Big numbers show as `~2^n`. The whole census is [saved with the world](persistence.md), so it survives a server restart. See [Finding & Gathering](gathering.md). |
| `/bot config reload` | Re-read the bot's [configuration](configuration.md) from disk and apply it without a server restart (`reload` is the only subcommand today). It echoes back a few of the freshly-loaded values so you can see the change took. |
...
- **`/bot trace`** — stop the bot and run a single block-level search from where it
  ...
  was accepted or rejected, to `orebit-trace.txt`. You can aim it without moving yourself:
  `/bot trace <x> <y> <z>` traces from the bot to that goal, and `/bot trace <from> <to>`
  traces between two coordinates you name. An offline analyzer renders the dump
...
- **`/bot debug on|off`** — toggle per-tick planner logging (skeleton dumps, window swaps,
  block-plan summaries), plus the particle overlay and chat progress, for watching the
  two-tier driver work in real time. Bare `/bot debug` flips it. It's a server-wide switch,
  so it works even with no bot spawned. **`/bot debug verbose on|off`** is a second, finer
  switch on top: it announces the movement the bot is currently executing, and the medium
  it's in, each time either changes.
